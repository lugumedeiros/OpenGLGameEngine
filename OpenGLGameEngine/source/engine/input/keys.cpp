#include "../../../include/engine/input/keys.h"

KeyObj::KeyObj(std::string_view key, int glfwCode) :key(key), glfwCode(glfwCode) {
}

KeyObj::KeyObj(std::string_view key, int glfwCode, bool shouldRepeat, double firstRepeatDelay, double repeatDelay) 
	:key(key), glfwCode(glfwCode), shouldRepeat(shouldRepeat), firstRepeatDelay(firstRepeatDelay), repeatDelay(repeatDelay) {
}

void KeyObj::setRepeat(bool shouldRepeat, double firstRepeatDelay, double repeatDelay) {
	this->shouldRepeat = shouldRepeat;
	this->firstRepeatDelay = firstRepeatDelay;
	this->repeatDelay = repeatDelay;
	currentRepeatPos = 0.0;
}
...
bool KeyObj::press(double eventTime){
	double deltaTime = eventTime - lastEventTime;
